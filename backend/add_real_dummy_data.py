...
def create_real_dummy_data():
    """실제 DB에 더미 데이터 생성"""

    print("=== DB에 더미 데이터 생성 시작 ===")

    # 1. 사용자 및 프로필 생성
    print("1. 사용자 및 프로필 생성 중...")
    ...
    if created:
        ...
        user.save()
        print(f"✓ 일반 사용자 생성: {user.username}")
    ...
    if created:
        print(f"✓ 사용자 프로필 생성: {user_profile}")
    ...
    if created:
        ...
        agent_user.save()
        print(f"✓ 중개사 사용자 생성: {agent_user.username}")
    ...
    if created:
        print(f"✓ 중개사 프로필 생성: {agent_profile}")
    ...
    for i, prop_data in enumerate(properties_data, 1):
        ...
        if created:
            ...
            print(f"✓ 매물 이미 존재: {title} - {prop_data['주소']}")

    print(f"\n=== 생성 완료: 총 {len(created_properties)}개의 매물이 추가되었습니다 ===")

    # 생성된 데이터 확인
    total_properties = Property.objects.count()
    print(f"현재 DB 총 매물 수: {total_properties}")

    return created_properties

if __name__ == '__main__':
    try:
        create_real_dummy_data()
        print("\n🎉 DB 더미 데이터 생성 완료!")
